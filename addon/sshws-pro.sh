...
action_install_cron() {
  echo -e "${CYAN}[*]${NC} Memasang cron: session-limiter (2m), trial-cleanup (1m), bandwidth-quota (5m), resource-monitor (10m)..."
  crontab -l 2>/dev/null | grep -v "$CRON_MARKER" > /tmp/crontab.tmp
  {
    echo "*/2 * * * * bash $SCRIPT_DIR/addon/session-limiter.sh >> /var/log/vpn-session-limiter.log 2>&1 $CRON_MARKER"
    echo "* * * * * bash $SCRIPT_DIR/addon/trial-cleanup.sh >> /var/log/vpn-trial-cleanup.log 2>&1 $CRON_MARKER"
    echo "*/5 * * * * bash $SCRIPT_DIR/addon/bandwidth-quota.sh >> /var/log/vpn-bandwidth-quota.log 2>&1 $CRON_MARKER"
    echo "*/10 * * * * bash $SCRIPT_DIR/addon/resource-monitor.sh >> /var/log/vpn-resource-monitor.log 2>&1 $CRON_MARKER"
  } >> /tmp/crontab.tmp
  crontab /tmp/crontab.tmp
  rm -f /tmp/crontab.tmp
  echo -e "  ${GREEN}[OK]${NC} 4 cron job terpasang"
}
...
action_install() {
  ...
  if ! command -v qrencode >/dev/null 2>&1; then
    echo -e "${CYAN}[*]${NC} Menginstall qrencode (buat QR code config)..."
...
action_diagnostic() {
  ...
  if crontab -l 2>/dev/null | grep -q "trial-cleanup"; then
    echo -e "  Cron trial-cleanup  : ${GREEN}terpasang${NC}"
  else
    echo -e "  Cron trial-cleanup  : ${RED}TIDAK terpasang${NC}"
  fi
  if crontab -l 2>/dev/null | grep -q "bandwidth-quota"; then
    echo -e "  Cron bandwidth-quota: ${GREEN}terpasang${NC}"
  else
    echo -e "  Cron bandwidth-quota: ${RED}TIDAK terpasang${NC}"
  fi
  if crontab -l 2>/dev/null | grep -q "resource-monitor"; then
    echo -e "  Cron resource-monitor: ${GREEN}terpasang${NC}"
  else
    echo -e "  Cron resource-monitor: ${RED}TIDAK terpasang${NC}"
  fi
  if command -v qrencode >/dev/null 2>&1; then
    echo -e "  QR Code (qrencode)  : ${GREEN}terinstall${NC}"
  else
    echo -e "  QR Code (qrencode)  : ${RED}belum terinstall${NC}"
  fi
