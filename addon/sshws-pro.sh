...
action_harden_all() {
  ...
  for svc in dropbear stunnel4 ws-dropbear ws-openssh haproxy xray nginx; do
    harden_service "$svc"
  done
...
action_install() {
  ...
  if [[ ! -f /usr/local/bin/ws-dropbear || ! -f /usr/local/bin/ws-openssh ]]; then
    echo -e "${CYAN}[1/4]${NC} Menginstall komponen dasar SSH-WS/SSH-SSL..."
    bash "$SCRIPT_DIR/addon/install-sshws.sh"
  else
    echo -e "${CYAN}[1/4]${NC} Komponen dasar SSH-WS/SSH-SSL sudah ada, skip."
  fi
  echo ""

  echo -e "${CYAN}[2/4]${NC} Mengaktifkan multiplex 443 (SNI bebas, routing by payload)..."
  bash "$SCRIPT_DIR/addon/enable-ssl-multiplex.sh" enable
  echo ""

  echo -e "${CYAN}[3/4]${NC} Hardening systemd (auto-restart semua service)..."
  action_harden_all
  echo ""

  echo -e "${CYAN}[4/4]${NC} Setup Session Limit & Telegram..."
...
action_diagnostic() {
  ...
  for svc in nginx dropbear stunnel4 haproxy ws-dropbear ws-openssh xray; do
