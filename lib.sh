...
# ─── PRO Config (session limit default, Telegram bot) ──────
...
# ─── QR Code (dirender langsung di terminal pakai qrencode) ──
show_qr() {
  local data="$1"
  ...
  if command -v qrencode >/dev/null 2>&1; then
    ...
    echo -e "  ${YELLOW}[!] qrencode gagal diinstall, QR tidak bisa ditampilkan${NC}"
  fi
}
...
count_vmess() {
  [[ -f "$DB_VMESS" ]] && wc -l < "$DB_VMESS" || echo 0
}
...
count_vless() {
  [[ -f "$DB_VLESS" ]] && wc -l < "$DB_VLESS" || echo 0
}
...
count_trojan() {
  [[ -f "$DB_TROJAN" ]] && wc -l < "$DB_TROJAN" || echo 0
}
...
DB_TRIAL="$DB_DIR/trial.db"

# ─── Trial Akun (durasi jam/menit, presisi lewat epoch timestamp) ──
create_trial_ssh() {
  local minutes="$1"
  local username="trial$(shuf -i 1000-9999 -n1)"
  local password=$(gen_ssh_password)
  local expiry_epoch=$(( $(date +%s) + minutes * 60 ))

  # Account expiry Linux itu presisi HARI, jadi kita kasih 1 hari sbg
  # jaring pengaman OS-level, sementara penghapusan presisi menit
  # ditangani cron (trial-cleanup.sh) berdasarkan expiry_epoch di trial.db.
  create_ssh "$username" "1" "$password" "1" >/dev/null

  touch "$DB_TRIAL"
  echo "$username|$expiry_epoch" >> "$DB_TRIAL"
...
cleanup_expired_trials() {
  ...
  local tmp="$DB_TRIAL.tmp"
  > "$tmp"
  while IFS='|' read -r user expiry; do
    [[ -z "$user" ]] && continue
    if [[ "$expiry" -le "$now" ]]; then
      delete_ssh "$user"
      echo "[$(date)] Trial expired & dihapus: $user"
      tg_notify "⏱️ <b>Trial Akun Habis</b>

Username: <code>$user</code>
Domain: <code>$domain</code>"
    else
      echo "$user|$expiry" >> "$tmp"
    fi
  done < "$DB_TRIAL"
  mv "$tmp" "$DB_TRIAL"
}
...
create_ssh() {
  ...
  local quota_mb="${5:-${QUOTA_MB_DEFAULT:-0}}"
  local exp=$(get_exp_date "$days")
  local created=$(date +"%Y-%m-%d")
  ...
  ensure_quota_chain "$username" 2>/dev/null
  echo "$password"
}

# ─── Kuota Bandwidth (akunting per-user via iptables owner-match) ──
DB_USAGE="$DB_DIR/usage.db"

ensure_quota_chain() {
  local username="$1"
  local chain="quota_${username}"
  command -v iptables >/dev/null 2>&1 || return 0
  iptables -N "$chain" 2>/dev/null
  iptables -C OUTPUT -m owner --uid-owner "$username" -j "$chain" 2>/dev/null || \
    iptables -I OUTPUT -m owner --uid-owner "$username" -j "$chain" 2>/dev/null
...
remove_quota_chain() {
  ...
  local chain="quota_${username}"
  command -v iptables >/dev/null 2>&1 || return 0
  iptables -D OUTPUT -m owner --uid-owner "$username" -j "$chain" 2>/dev/null
...
#  AUTO-UPDATE
# ════════════════════════════════════════════════════════════
UPDATE_RAW="https://raw.githubusercontent.com/chanelog/VpnP/main"
