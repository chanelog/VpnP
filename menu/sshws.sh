...
sshws_menu() {
  ...
  echo -e "  ${CYAN}$LINE${NC}"
  echo -e "  ${GREEN}[1t]${NC} Buat Trial Akun (30m/1h/3h/1d)"
  echo -e "  ${CYAN}$LINE${NC}"
  ...
  echo -e "  ${CYAN}$LINE${NC}"
  echo -e "  ${PURPLE}[7]${NC}  Pengaturan Pro (Session Limit & Telegram)"
  echo -e "  ${CYAN}$LINE${NC}"
  echo -e "  ${PURPLE}[8]${NC}  Diagnostic (cek status semua fitur)"
  echo -e "  ${CYAN}$LINE${NC}"
  ...
  case "$choice" in
    ...
    5) do_renew_ssh ;;
    6) do_list_ssh ;;
    7) bash "$SCRIPT_DIR/addon/sshws-pro.sh" config; load_pro_config; echo -ne "  ${DIM}Tekan Enter...${NC}"; read -r; sshws_menu ;;
    8) bash "$SCRIPT_DIR/addon/sshws-pro.sh" diagnostic; echo -ne "  ${DIM}Tekan Enter...${NC}"; read -r; sshws_menu ;;
...
do_create_ssh() {
  ...
  echo -ne "  ${YELLOW}Limit sesi aktif${NC} [default: ${SESSION_LIMIT_DEFAULT:-2}, 0=unlimited]: "
  ...
  echo -ne "  ${YELLOW}Kuota bandwidth (MB)${NC} [default: ${QUOTA_MB_DEFAULT:-0}, 0=unlimited]: "
  read -r quota_mb
  quota_mb="${quota_mb:-${QUOTA_MB_DEFAULT:-0}}"
  [[ ! "$quota_mb" =~ ^[0-9]+$ ]] && quota_mb="${QUOTA_MB_DEFAULT:-0}"

  local pass
  pass=$(create_ssh "$username" "$days" "$password" "$session_limit" "$quota_mb")
  local domain=$(get_domain)
  local exp=$(get_exp_date "$days")

  tg_notify "🆕 <b>Akun SSH Baru</b>

Username: <code>$username</code>
Password: <code>$pass</code>
Domain: <code>$domain</code>
Expired: <code>$exp</code> ($days hari)
Limit sesi: <code>$session_limit</code>"
  ...
  echo -e "  ${YELLOW}Limit Sesi ${NC}: ${WHITE}$session_limit${NC} $([[ "$session_limit" == "0" ]] && echo "(unlimited)")"
  echo -e "  ${YELLOW}Kuota     ${NC}: ${WHITE}$([[ "$quota_mb" == "0" ]] && echo "unlimited" || echo "${quota_mb}MB")${NC}"
  echo -e "${CYAN}$LINE${NC}"
...
do_create_trial() {
  sshws_header
  echo ""
  echo -e "  ${WHITE}BUAT TRIAL AKUN${NC}"
  echo -e "  ${CYAN}$LINE${NC}"
  echo ""
  echo -e "  ${YELLOW}[1]${NC} 30 menit   ${YELLOW}[2]${NC} 1 jam   ${YELLOW}[3]${NC} 3 jam   ${YELLOW}[4]${NC} 1 hari"
  echo -ne "  ${WHITE}Pilih durasi${NC}: "
  read -r d
  local minutes
  case "$d" in
    1) minutes=30 ;;
    ...
    3) minutes=180 ;;
    4) minutes=1440 ;;
    *) echo -e "  ${RED}[!] Pilihan tidak valid${NC}"; sleep 1; sshws_menu; return ;;
  esac

  local result
  result=$(create_trial_ssh "$minutes")
  ...
  clear
  echo -e "${CYAN}$LINE${NC}"
  echo -e "${WHITE}           ✓  TRIAL AKUN DIBUAT${NC}"
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}Username${NC}: ${WHITE}$username${NC}"
  echo -e "  ${YELLOW}Password${NC}: ${WHITE}$pass${NC}"
  echo -e "  ${YELLOW}Domain  ${NC}: ${WHITE}$domain${NC}"
  echo -e "  ${YELLOW}Durasi  ${NC}: ${WHITE}$minutes menit${NC} ${DIM}(auto-hapus tepat waktu)${NC}"
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${WHITE}SSH-SSL${NC}: port $STUNNEL_SSL_PORT / ${HAPROXY_SSL_PORT:-444}, SNI bebas, tanpa payload"
  echo -e "  ${WHITE}SSH-WS TLS${NC}: port 443, path /ssh-ws"
  echo -e "${CYAN}$LINE${NC}"
  echo ""
  echo -ne "  ${DIM}Tekan Enter untuk kembali...${NC}"; read -r
  sshws_menu
...
do_info_ssh() {
  ...
  quota_mb="${quota_mb:-${QUOTA_MB_DEFAULT:-0}}"
  local used_bytes=$(grep "^$username|" "$DB_USAGE" 2>/dev/null | cut -d'|' -f2)
  ...
  echo -e "  ${YELLOW}Sesi Aktif${NC}: ${WHITE}$active${NC} / ${WHITE}$([[ "$slimit" == "0" ]] && echo "unlimited" || echo "$slimit")${NC}"
  echo -e "  ${YELLOW}Kuota     ${NC}: ${WHITE}${used_mb}MB${NC} / ${WHITE}$([[ "$quota_mb" == "0" ]] && echo "unlimited" || echo "${quota_mb}MB")${NC}"
  echo -e "  ${CYAN}$LINE${NC}"
  echo ""
  echo -ne "  ${DIM}Tekan Enter untuk kembali...${NC}"; read -r
  sshws_menu
...
do_renew_ssh() {
  ...
  sed -i "/^$username|/d" "$DB_USAGE" 2>/dev/null
  echo -e "  ${GREEN}[✓] Diperpanjang hingga ${WHITE}$(get_exp_date "$days")${NC} ${DIM}(akun di-unlock & kuota direset)${NC}"; sleep 2; sshws_menu
}
